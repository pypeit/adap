...
**2. Get raw data in.** [adap_koa_download.yml](nautilus_jobs/adap_koa_download.yml) pulls from the Keck Observatory Archive straight into `s3://pypeit/adap_2023/raw_data_reorg/`, the root `get_cloud_path` hands to every stage in the workflow. [scripts/download_lib/](scripts/download_lib/) is the target-name/coordinate-driven KOA querier that does the work and lays out the `<target>/<YYYYMMDD>/<LRIS|LRISBLUE>/raw_[rb]` tree. A **dataset** on this branch is that three-part path, and the red and blue arms are separate datasets.
...
- **The sheet is addressed by name, not by key.** Every job now passes `Scorecard/<tab>`, so the scorecard tabs — which are resolved from whichever spreadsheet the job was handed — all land in the same place. The one exception is the stale [adap-reduce-from-queue.yml](nautilus_jobs/adap-reduce-from-queue.yml), which still carries a `key=15ealTQOBLB0I…` argument and should be deleted.
