...
**2. Get raw data in.** [adap_koa_download.yml](nautilus_jobs/adap_koa_download.yml) pulls from the Keck Observatory Archive straight into `s3://pypeit/adap_2023/raw_data_reorg/`, the root `get_cloud_path` hands to the reduce and post-processing stages — with the exception of [coadd2d_from_queue.py](scripts/coadd2d_from_queue.py), which builds its own path and reads a different root. [scripts/download_lib/](scripts/download_lib/) is the target-name/coordinate-driven KOA querier that does the work and lays out the `<target>/<YYYYMMDD>/<LRIS|LRISBLUE>/raw_[rb]` tree. A **dataset** on this branch is that three-part path, and the red and blue arms are separate datasets.
