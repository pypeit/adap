...
def initialize_cloud_storage(cloud_uri, args):
    ...
    if scheme == 's3' and _s3_support_enabled:
        ...
        raise ValueError(f"Cannot initialize cloud storage, {cloud_uri} is not a URI")
    else:
        raise RuntimeError(f"Cannot initialize cloud storage, {scheme} is not a supported cloud URI scheme.")
...
def list_objects(cloud_uri, patterns=[]):
    ...
    if parsed_uri.scheme == 's3' and _s3_client is not None:
        ...
        raise RuntimeError(f"Cannot initialize cloud storage, {parsed_uri.scheme} is not a supported cloud URI scheme.")
...
def upload(source_file, dest_uri):
    ...
    return _s3_client.upload_file(str(source_file), parsed_dest.netloc, parsed_dest.path.lstrip('/'))
...
def download(source_uri, dest_dir):
    ...
    dest_file = dest_dir / parsed_source.path.split("/")[-1]

    _s3_client.download_file(parsed_source.netloc, parsed_source.path.lstrip('/'), str(dest_file) )
    return dest_file

def delete(source_uri):
    if _s3_client is None:
        raise RuntimeError(f"Cannot delete {source_uri}: S3 cloud storage has not been initialized.")

    parsed_source = urllib.parse.urlsplit(source_uri)    
    if parsed_source.scheme != 's3':
        raise ValueError(f"Cannot download from {source_uri}: only S3 uploads are supported.")

    _s3_client.delete_object(Bucket=parsed_source.netloc, Key=parsed_source.path.lstrip('/'))
